...
/// <summary>
/// Server reveal of the map row now visible to the south after a step (s2c 0x67).
/// </summary>
public sealed class GameServerMapBottomRowMessage : IProtocolMessage
{
    private const int FloorWidth = 18;

    public GameServerMapBottomRowMessage(IReadOnlyList<IReadOnlyList<MapTile?>> floors)
    {
        Floors = floors;
    }

    public byte Opcode => (byte)GameServerOpcode.MapBottomRow;
    ...
    public static IProtocolMessage Read(GameServerOpcode opcode, PacketReader reader, IItemTypeProvider itemTypes, byte currentZ)
    {
        ...
        return new GameServerMapBottomRowMessage(floors);
    }
}
