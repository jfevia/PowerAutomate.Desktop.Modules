...
public class SelectListBoxItemAction : InputSimulationActionBase
{
    public SelectListBoxItemAction()
    {
    }
    ...
    protected override void Run(ActionContext context)
    {
        ...
        var index = ListSelection.Resolve(
            ...
            Index,
            Text,
            WindowMessages.ListBoxFindStringExact,
            WindowMessages.ListBoxSetCurrentSelection,
            WindowMessages.ListBoxError,
            "list box");

        // LB_SETCURSEL updates the control without telling the parent, so the notification is sent explicitly.
        NotificationSender.NotifyParent(Context, handle, WindowMessages.NotifyListBoxSelectionChange);
        SelectedIndex = index;
    }
}
