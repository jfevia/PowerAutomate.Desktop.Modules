...
public class ClickControlAction : ActionBase
{
    ...
    public override void Execute(ActionContext context)
    {
        try
        {
            if (Control is null)
            {
                throw new ArgumentException("A control is required to send a click.", nameof(Control));
            }

            var handle = Control.NativeHandle;
            WindowExtensions.GetCenter(handle, out var centerX, out var centerY);

            InputSender.Click(handle, Button, X ?? centerX, Y ?? centerY, false);
        }
        catch (Exception ex)
        {
