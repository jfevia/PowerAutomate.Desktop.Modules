...
[TestFixture]
public class DeleteTaskActionTests
{
    [SetUp]
    ...
    [Test]
    public void Execute_DeletesTaskFromItsFolder()
    {
        var folder = new FakeTaskFolder();
        _test.Task.Folder = folder;
        var action = new DeleteTaskAction(_test.Context) { TaskName = "Task" };

        action.Execute(ActionHarness.ActionContext());

        Assert.That(folder.DeletedTaskName, Is.EqualTo("Task"));
    }

    [Test]
    public void Execute_WhenTaskIsMissing_ThrowsTaskNotFoundError()
    {
        var action = new DeleteTaskAction(_test.Context) { TaskName = "Missing" };
    ...
    [Test]
    public void Execute_WhenFolderReportsMissingTask_ThrowsTaskNotFoundError()
    {
        _test.Task.Folder = new FakeTaskFolder { DeleteThrowsFileNotFound = true };
        var action = new DeleteTaskAction(_test.Context) { TaskName = "Task" };
    ...
    [Test]
    public void DefaultConstructor_UsesProductionWiring() => Assert.That(new DeleteTaskAction().TaskName, Is.Null);
}
