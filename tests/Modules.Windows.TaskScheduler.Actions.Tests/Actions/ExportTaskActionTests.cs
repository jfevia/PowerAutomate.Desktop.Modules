...
[TestFixture]
public class ExportTaskActionTests
{
    [SetUp]
    ...
    [Test]
    public void Execute_ExportsTask()
    {
        var action = new ExportTaskAction(_test.Context) { TaskName = "Task", FileName = "task.xml" };

        action.Execute(ActionHarness.ActionContext());

        Assert.That(_test.Task.ExportedFileName, Is.EqualTo("task.xml"));
    }

    [Test]
    public void Execute_WhenTaskIsMissing_ThrowsTaskNotFoundError()
    {
        var action = new ExportTaskAction(_test.Context) { TaskName = "Missing" };
    ...
    [Test]
    public void DefaultConstructor_UsesProductionWiring() => Assert.That(new ExportTaskAction().TaskName, Is.Null);
}
