...
[TestFixture]
public class StartTaskActionTests
{
    [SetUp]
    ...
    [Test]
    public void Execute_RunsTask()
    {
        var action = new StartTaskAction(_test.Context) { TaskName = "Task" };

        action.Execute(ActionHarness.ActionContext());

        Assert.That(_test.Task.Ran, Is.True);
    }

    [Test]
    public void Execute_WhenTaskIsMissing_ThrowsTaskNotFoundError()
    {
        var action = new StartTaskAction(_test.Context) { TaskName = "Missing" };
    ...
    [Test]
    public void DefaultConstructor_UsesProductionWiring() => Assert.That(new StartTaskAction().TaskName, Is.Null);
}
