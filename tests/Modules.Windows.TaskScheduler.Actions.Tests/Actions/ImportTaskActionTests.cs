...
[TestFixture]
public class ImportTaskActionTests
{
    [SetUp]
    ...
    [Test]
    public void Execute_ImportsTask()
    {
        var action = new ImportTaskAction(_test.Context) { TaskName = "Imported", FileName = "task.xml" };

        action.Execute(ActionHarness.ActionContext());

        Assert.That(_test.Service.Root.ImportedTaskName, Is.EqualTo("Imported"));
        Assert.That(_test.Service.Root.ImportedFileName, Is.EqualTo("task.xml"));
    }
    ...
    public void Execute_WhenConnectionFails_ThrowsUnknownError()
    {
        ...
        var action = new ImportTaskAction(_test.Context);
    ...
    [Test]
    public void DefaultConstructor_UsesProductionWiring() => Assert.That(new ImportTaskAction().TaskName, Is.Null);
}
