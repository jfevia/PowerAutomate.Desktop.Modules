...
[TestFixture]
public class ToggleTaskActionTests
{
    [SetUp]
    ...
    [Test]
    public void Execute_TogglesTask()
    {
        var action = new ToggleTaskAction(_test.Context) { TaskName = "Task", Enabled = true };

        action.Execute(ActionHarness.ActionContext());

        Assert.That(_test.Task.Enabled, Is.True);
    }

    [Test]
    public void Execute_WhenTaskIsMissing_ThrowsTaskNotFoundError()
    {
        var action = new ToggleTaskAction(_test.Context) { TaskName = "Missing" };
    ...
    [Test]
    public void DefaultConstructor_UsesProductionWiring() => Assert.That(new ToggleTaskAction().TaskName, Is.Null);
}
