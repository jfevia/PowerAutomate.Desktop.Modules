...
{
    [Test]
    public void Opcode_IsMove()
    {
        var message = new ClientMoveMessage(new Position(1, 2, 3), 10, 4, new Position(5, 6, 7), 9);

        Assert.That(message.Opcode, Is.EqualTo((byte)ClientOpcode.Move));
    }
    ...
    public void Constructor_KeepsFields()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(message.Count, Is.EqualTo(9));
        });
    }

    [Test]
    public void Write_EmitsFromThenItemIdThenStackThenToThenCount()
    {
        var writer = new PacketWriter();

        new ClientMoveMessage(new Position(1, 2, 3), 10, 4, new Position(5, 6, 7), 9).Write(writer);

        Assert.That(writer.ToArray(), Is.EqualTo(new byte[]
        {
            (byte)ClientOpcode.Move,
            1, 0, 2, 0, 3,
            10, 0,
            4,
            5, 0, 6, 0, 7,
