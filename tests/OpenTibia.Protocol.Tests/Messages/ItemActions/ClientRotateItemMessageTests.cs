...
[TestFixture]
public class ClientRotateItemMessageTests
{
    [Test]
    public void Opcode_IsRotateItem()
    {
        var message = new ClientRotateItemMessage(new Position(1, 2, 3), 10, 4);

        Assert.That(message.Opcode, Is.EqualTo((byte)ClientOpcode.RotateItem));
    }
    ...
    [Test]
    public void Write_EmitsPositionThenItemIdThenStack()
    {
        var writer = new PacketWriter();

        new ClientRotateItemMessage(new Position(1, 2, 3), 10, 4).Write(writer);

        Assert.That(writer.ToArray(), Is.EqualTo(new byte[]
        {
            (byte)ClientOpcode.RotateItem,
            1, 0, 2, 0, 3,
            10, 0,
            4
        }));
