...
[TestFixture]
public class ClientLookMessageTests
{
    [Test]
    public void Opcode_IsLook()
    {
        var message = new ClientLookMessage(new Position(1, 2, 3), 10, 4);

        Assert.That(message.Opcode, Is.EqualTo((byte)ClientOpcode.Look));
    }
    ...
    public void Write_EmitsPositionThenItemIdThenStack()
    {
        ...
        new ClientLookMessage(new Position(1, 2, 3), 10, 4).Write(writer);

        Assert.That(writer.ToArray(), Is.EqualTo(new byte[]
        {
            (byte)ClientOpcode.Look,
            1, 0, 2, 0, 3,
            10, 0,
            4
        }));
