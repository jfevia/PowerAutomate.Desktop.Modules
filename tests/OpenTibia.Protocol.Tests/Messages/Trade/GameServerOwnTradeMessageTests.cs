...
[TestFixture]
public class GameServerOwnTradeMessageTests
{
    ...
    public void Read_WithItems_ParsesPlayerNameThenEveryItem()
    {
        ...
        var message = (GameServerOwnTradeMessage)GameServerOwnTradeMessage.Read(GameServerOpcode.OwnTrade, reader, ItemTypes);

        Assert.Multiple(() =>
        {
            Assert.That(message.Opcode, Is.EqualTo((byte)GameServerOpcode.OwnTrade));
            Assert.That(message.PlayerName, Is.EqualTo("Gamemaster"));
    ...
    {
        Assert.Throws<ArgumentNullException>(() => new GameServerOwnTradeMessage(null!, Array.Empty<ItemStack>()));
    }

    [Test]
    public void Constructor_WithNullItems_Throws()
    {
        Assert.Throws<ArgumentNullException>(() => new GameServerOwnTradeMessage(string.Empty, null!));
    }

    [Test]
    public void Read_WithNullReader_Throws()
    {
        Assert.Throws<ArgumentNullException>(() => GameServerOwnTradeMessage.Read(GameServerOpcode.OwnTrade, null!, ItemTypes));
    }
    ...
    public void Read_WithNullItemTypes_Throws()
    {
        Assert.Throws<ArgumentNullException>(() =>
            GameServerOwnTradeMessage.Read(GameServerOpcode.OwnTrade, new PacketReader(new byte[3]), null!));
    }
    ...
    public void Read_WithTruncatedPayload_ThrowsProtocolException()
    {
        ...
        Assert.Throws<ProtocolException>(() => GameServerOwnTradeMessage.Read(GameServerOpcode.OwnTrade, reader, ItemTypes));
    }
}
